        ...
        {sorted.map((biz: any) => {
          ...
          return (
            <div key={biz.id} style={{ backgroundColor: isDowntown ? '#f0f7ff' : 'white', borderRadius: '16px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: isFeatured ? '2px solid ' + meta.color : isDowntown ? '1px solid #93c5fd' : '1px solid #f1f5f9' }}>
              ...
              {!biz.is_claimed ? <button onClick={() => setClaimBiz({id: biz.id, name: biz.name})} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', backgroundColor: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '12px', height: '38px', minHeight: 0, color: 'rgba(255,255,255,0.6)', fontSize: '12px', fontWeight: 600, cursor: 'pointer', width: '100%' }}><Shield size={13} />Is this your business? Claim it free</button> : <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', height: '32px' }}><CheckCircle size={13} color='#4ade80' /><span style={{ fontSize: '12px', color: '#4ade80', fontWeight: 600 }}>Verified Owner</span></div>}
