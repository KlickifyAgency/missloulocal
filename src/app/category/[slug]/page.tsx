          ...
          return (
            <div key={biz.id} style={{ backgroundColor: isDowntown ? '#f0f7ff' : 'white', borderRadius: '16px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: isFeatured ? '2px solid ' + meta.color : isDowntown ? '1px solid #93c5fd' : '1px solid #f1f5f9' }}>
              ...
              {biz.is_claimed
                ...
                : <button onClick={() => setClaimBiz({id: biz.id, name: biz.name})} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', backgroundColor: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: '12px', height: '40px', minHeight: 0, color: '#64748b', fontSize: '13px', fontWeight: 600, cursor: 'pointer', width: '100%' }}><Shield size={13} strokeWidth={2} />Is this your business? Claim it free</button>
              }
