...
import { Wrench, UtensilsCrossed, HeartPulse, Church, Home, CalendarDays, Scale, Car, Scissors, PawPrint, Settings, ShoppingBag, Palette, Compass, Footprints } from 'lucide-react'
      ...
      <div style={{ padding: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          {categories.map(({ name, slug, icon: Icon, color, bg }) => (
            <Link key={slug} href={'/category/' + slug} style={{ backgroundColor: 'white', borderRadius: '16px', padding: '20px 14px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', textDecoration: 'none', minHeight: 0, height: 'auto', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #f1f5f9' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#334155', lineHeight: 1.3, textAlign: 'center' }}>{name}</span>
            </Link>
          ))}
        </div>
      </div>
